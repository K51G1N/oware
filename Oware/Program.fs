...
let incrementScore lastHousePlaced turn board= 
  ...
  let rec scoreAdder  lastHousePlaced board =
    ...
    let pl2NumPieces = board.PlayerB.nrSeeds
    match lastHousePlaced<13 && lastHousePlaced>0 with //makes sure lastHouse is valid
    |true ->
        match turn=North with
        |true -> match getSeeds lastHousePlaced board, lastHousePlaced<7 && lastHousePlaced>0, (plANumPieces - 2 > 0 || (plANumPieces = 2 && pl2NumPieces = 0)), (plANumPieces - 3 > 0 || (plANumPieces = 3 && pl2NumPieces = 0)) with //if it is north's turn, seeds should only be collected from south's side
                 ...
                 | _ -> board
        |_  -> match getSeeds lastHousePlaced board, lastHousePlaced<13 && lastHousePlaced>6, (pl2NumPieces - 2 > 0 || (pl2NumPieces = 2 && plANumPieces = 0)), (pl2NumPieces - 3 > 0 || (pl2NumPieces = 3 && plANumPieces = 0)) with //if it is south's turn, seeds should only be collected from north's side
...
let main _ =
   // g17e4476: Get's us into the game
    let beginning = playGame (start North)
    0
